...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithViper("TRST") // In simapp, we don't use any prefix for env variables.

	rootCmd := &cobra.Command{
		Use:   "trstd",
		Short: "Trustless Hub App Daemon",
...
func appExport(
	...
	if height != -1 {
		App = app.NewTrstApp(logger, db, traceStore, false, appOpts)

		if err := App.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		App = app.NewTrstApp(logger, db, traceStore, true, appOpts)
	}
