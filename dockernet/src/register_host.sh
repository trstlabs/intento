...
$TRST_MAIN_CMD tx stakeibc register-host-zone \
    $CONNECTION $HOST_DENOM $ADDRESS_PREFIX $IBC_DENOM $CHANNEL 1 \
    --gas 1000000 --from $TRST_ADMIN_ACCT --home $DOCKERNET_HOME/state/trst1 -y | TRIM_TX
sleep 10
...
$TRST_MAIN_CMD tx stakeibc add-validators $CHAIN_ID $validator_json \
    --from $TRST_ADMIN_ACCT -y | TRIM_TX
sleep 5

# Confirm the ICA accounts have been registered before continuing
timeout=100
while true; do
    if ! $TRST_MAIN_CMD q stakeibc show-host-zone $CHAIN_ID | grep Account | grep -q null; then
        break
    else
        if [[ "$timeout" == "0" ]]; then 
