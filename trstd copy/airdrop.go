...
const (
	MaxCap                 = 50000000000
	TotalTrstAirdropAmount = 1000000000000 // 0.5% * 200000000
)
...
Example:
trstd export-airdrop-snapshot ./genesis.cosmoshub-4.json ./genesis_secret_3.json  ./snapshot.json
	- Check input genesis:
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			snapshot = exportSnapShotFromGenesisFile(clientCtx, firstGenesisFile, "uatom", snapshotOutput, snapshot)
			snapshotSecret := exportSecretSnapShotFromGenesisFile(clientCtx, secondGenesisFile, "uscrt", snapshotOutput, snapshot)

			fmt.Printf("atom amount %s", snapshot.TotalAtomAmount)
			fmt.Printf("scrt amount %s", snapshotSecret.TotalAtomAmount)
...
func exportSecretSnapShotFromGenesisFile(clientCtx client.Context, genesisFile string, denom string, snapshotOutput string, snapshot Snapshot) Snapshot {
	appState, _, _ := genutiltypes.GenesisStateFromGenFile(genesisFile)
