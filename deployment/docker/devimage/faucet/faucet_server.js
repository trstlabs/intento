...
const DENOM = process.env.DENOM || "utrst";
...
async function send_command(src_key_name, src_address, dest_address, amount) {
  const send_message = `trstd tx bank send ${src_address} ${dest_address} ${amount}${DENOM} --from ${src_key_name} --gas-prices 0.25utrst -y`;
...
async function get_address(key_name) {
  ...
  const list_keys = "trstd keys list";
