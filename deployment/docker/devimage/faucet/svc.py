...
chain_id = os.getenv("CHAIN_ID", 'trst_chain_1')

def get_address(key_name: str) -> str:
    p = subprocess.check_output(['trstd', 'keys', 'list'])
    print(f'{p}')
    res = ''
...
def send_command(src: str, dest: str, amount: str) -> str:
    exec = ['trstd', 'tx', 'bank', 'send' ]

    address = get_address(src)

    exec.append(address)
    exec.append(dest)
    exec.append(f'{amount}utrst')
    exec.extend(['--from', src, '--gas-prices', '0.25utrst', '-y'])
    return exec
