...
of the certificate created by the `tppd init-enclave` command
...
call to the `tppd`. This means we need some manual setup (to add SCRT to the service, and configure the node):

`tppd config chain-id <chain-id>`
