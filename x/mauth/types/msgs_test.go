...
import (
	...
	"github.com/trstlabs/trst/x/mauth/types"
)
...
func TestMsgSubmitTxValidateBasic(t *testing.T) {
	...
	for i, tc := range testCases {
		ownerAddr, err := sdk.AccAddressFromBech32(TestOwnerAddress)
		require.NoError(t, err)
		msg, _ = types.NewMsgSubmitTx(ownerAddr, TestMessage, ibctesting.FirstConnectionID, ibctesting.FirstConnectionID)
...
func TestMsgSubmitTxGetSigners(t *testing.T) {
	...
	msg, err := types.NewMsgSubmitTx(expSigner, TestMessage, ibctesting.FirstConnectionID, ibctesting.FirstConnectionID)
	require.NoError(t, err)
