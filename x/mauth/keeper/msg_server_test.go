...
import (
	...
	"github.com/trstlabs/trst/x/mauth/keeper"
...
func (suite *KeeperTestSuite) TestRegisterInterchainAccount() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			msgSrv := keeper.NewMsgServerImpl(*GetICAApp(suite.chainA).AppKeepers.ICAAuthKeeper)
...
func (suite *KeeperTestSuite) TestSubmitTx() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			msgSrv := keeper.NewMsgServerImpl(*icaAppA.AppKeepers.ICAAuthKeeper)
			msg, err := types.NewMsgSubmitTx(ownerAddr, icaMsg, connectionId, counterpartyConnectionId)
			suite.Require().NoError(err)
