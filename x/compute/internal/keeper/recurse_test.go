...
func initRecurseContract(t *testing.T) (contract sdk.AccAddress, creator sdk.AccAddress, ctx sdk.Context, keeper Keeper) {
	...
	require.NoError(t, err)
	codeID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
...
//QueryMethodContractStateSmart undefined
			...
			"recursion 4, external gas limit": {
		   	...
		   	for name, tc := range cases {
		   		t.Run(name, func(t *testing.T) {
		   			...
		   			contractMsg := types.ContractMsg{
		   				CodeHash: []byte(hex.EncodeToString(keeper.GetContractHash(ctx, contractAddr))),
		   				Msg:      msg,
		   			}

		   			msg, err := wasmCtx.Encrypt(contractMsg.Serialize())
		   			require.NoError(t, err)

		   			// do the query
		   			path := []string{QueryGetContractPrivateState, contractAddr.String(), QueryMethodContractStateSmart}
