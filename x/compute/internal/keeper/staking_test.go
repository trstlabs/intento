...
// StakingExecuteMsg is used to encode handle messages
...
func TestInitializeStaking(t *testing.T) {
	...
	stakingID, err := keeper.Create(ctx, creator, stakingCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
	...
	badMsg := StakingMsg{
		...
		Symbol:       "MISS",
		Decimals:     0,
		Validator:    sdk.ValAddress(bob.Bytes()),
		ExitTax:      sdk.MustNewDecFromStr("0.10"),
		MinWithdrawl: "100",
	}
