...
func decryptAttribute(attr cosmwasm.Attribute, nonce []byte) (cosmwasm.Attribute, error) {
	...
	if err != nil {
		...
		return cosmwasm.Attribute{}, fmt.Errorf("Failed DecodeString for key %+v", attr.Key)
	}
	keyPlainBz, err := wasmCtx.Decrypt(keyCipherBz, nonce)
	if err != nil {
...
func parseAndDecryptAttributes(attrs []abci.EventAttribute, nonce []byte, shouldDecrypt bool) ([]cosmwasm.Attribute, error) {
...
func TestExternalQueryCalleeDoesntExist(t *testing.T) {
	for _, testContract := range testContracts {
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			...
			_, _, _, _, _, err = execHelper(t, keeper, ctx, addr, walletA, privKeyA, `{"send_external_query_error":{"to":"trust13l72vhjngmg55ykajxdnlalktwglyqjqv9pkq4","code_hash":"bla bla"}}`, true, testContract.IsCosmWasmV1, defaultGasForTests, 0)
		...
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			...
			/*
				t.Run("EmptyCodeHash", func(t *testing.T) {
					_, _, _, _, err := initHelperImpl(t, keeper, ctx, codeID, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"","msg":"%s","contract_id":"2"}}`, codeID, `{\"nop\":{}}`), false, testContract.IsCosmWasmV1, defaultGasForTests, 2, sdk.NewCoins())
				...
				t.Run("TooBigCodeHash", func(t *testing.T) {
						_, _, _, _, err := initHelperImpl(t, keeper, ctx, codeID, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"%sa","msg":"%s","contract_id":"3"}}`, codeID, codeHash, `{\"nop\":{}}`), true, testContract.IsCosmWasmV1, defaultGasForTests, 2, sdk.NewCoins())
					...
					t.Run("TooSmallCodeHash", func(t *testing.T) {
						_, _, _, _, err := initHelperImpl(t, keeper, ctx, codeID, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"%s","msg":"%s","contract_id":"4"}}`, codeID, codeHash[0:63], `{\"nop\":{}}`), false, testContract.IsCosmWasmV1, defaultGasForTests, 2, sdk.NewCoins())
					...
					t.Run("IncorrectCodeHash", func(t *testing.T) {
						_, _, _, _, err := initHelperImpl(t, keeper, ctx, codeID, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","msg":"%s","contract_id":"5"}}`, codeID, `{\"nop\":{}}`), false, testContract.IsCosmWasmV1, defaultGasForTests, 2, sdk.NewCoins())
...
func TestCodeHashExecCallInit(t *testing.T) {
	for _, testContract := range testContracts {
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			...
			t.Run("IncorrectCodeHash", func(t *testing.T) {
				_, _, _, _, _, err := execHelperImpl(t, keeper, ctx, addr, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855","msg":"%s","contract_id":"5"}}`, codeID, `{\"nop\":{}}`), false, testContract.IsCosmWasmV1, defaultGasForTests, 0, 2)
...
func TestContractIdCollisionWhenMultipleCallbacksToInitFromSameContract(t *testing.T) {
	for _, testContract := range testContracts {
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			...
			_, _, _, _, _, err = execHelperImpl(t, keeper, ctx, addr, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"%s","msg":"%s","contract_id":"1"}}`, codeID, codeHash, `{\"nop\":{}}`), true, testContract.IsCosmWasmV1, defaultGasForTests, 0, 2)
			require.Empty(t, err)

			_, _, _, _, _, err = execHelperImpl(t, keeper, ctx, addr, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d,"code_hash":"%s","msg":"%s","contract_id":"1"}}`, codeID, codeHash, `{\"nop\":{}}`), false, testContract.IsCosmWasmV1, defaultGasForTests, 0, 1)
			require.NotEmpty(t, err)
			require.NotNil(t, err.GenericErr)
		...
		t.Run(testContract.CosmWasmVersion, func(t *testing.T) {
			...
			t.Run("TooBigCodeHash", func(t *testing.T) {
				...
				if testContract.IsCosmWasmV1 {
					...
					require.Contains(t,
						err.Error(),
						"parsing test_contract::contract::ExecuteMsg: Expected to parse either a `true`, `false`, or a `null`.",
					)
				}
			})
			t.Run("TooSmallCodeHash", func(t *testing.T) {
...
func TestEncryptedAndPlaintextLogs(t *testing.T) {
	...
	require.Equal(t,
		[]ContractEvent{
			{
				{Key: "contract_address", Value: []byte(addr.String()), AccAddr: "", Encrypted: false, PubDb: false},
				{Key: "encrypted log", Value: []byte("encrypted value"), AccAddr: "", Encrypted: false, PubDb: false},
				{Key: "ZW5jb2RlZCBsb2cK", Value: []byte("ZW5jb2RlZCB2YWx1ZQo="), AccAddr: "", Encrypted: false, PubDb: false},
				{Key: "plaintext log", Value: []byte("plaintext value"), AccAddr: "", Encrypted: false, PubDb: false},
			},
		},
		events,
	)
...
func TestV010InitV1ContractFromInitWithErrResponse(t *testing.T) {
	...
	_, _, _, _, err = initHelper(t, keeper, ctx, v010CodeID, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d, "code_hash":"%s","contract_id":"blabla", "msg":"%s"}}`, codeID, codeHash, `{\"counter\":{\"counter\":0, \"expires\":100}}`), true, true, defaultGasForTests)
...
func TestV010InitV1ContractFromExecuteWithErrResponse(t *testing.T) {
	...
	_, _, _, _, _, err = execHelper(t, keeper, ctx, contractAddress, walletA, privKeyA, fmt.Sprintf(`{"call_to_init":{"code_id":%d, "code_hash":"%s","contract_id":"blabla", "msg":"%s"}}`, codeID, codeHash, `{\"counter\":{\"counter\":0, \"expires\":100}}`), true, true, defaultGasForTests, 0)
	require.Contains(t, fmt.Sprintf("%+v", err), "got wrong counter on init")
}
