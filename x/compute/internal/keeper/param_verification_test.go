...
func multisigTxCreatorForExisting(
	...
	case *types.MsgInstantiateContract:
		msg.Sender = multisigAccount.address.String()
...
func TestMultipleSigners(t *testing.T) {
	//
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, walletA, privKeyA, walletB, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.NewCoins())
	...
	contractAddressA, _, err := keeper.Instantiate(ctx, codeID, walletA initMsgBz, nil, "demo contract 1", sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
	if err != nil {
	...
	require.Equal(t,
		[]ContractEvent{
			{
				{Key: "contract_address", Value: contractAddressA.Bytes()},
				{Key: "init", Value: []byte("🌈")},
			},
		},
		wasmEvents,
	...
	contractAddressB, _, err := keeper.Instantiate(ctx, codeID, walletB initMsgBz, nil, "demo contract 2", sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
	if err != nil {
	...
	require.Equal(t,
		[]ContractEvent{
			...
			{
				{Key: "contract_address", Value: contractAddressB.Bytes()},
				{Key: "init", Value: []byte("🌈")},
			},
		},
		wasmEvents,
...
func TestWrongSigner(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, _, walletA, _, walletB, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.NewCoins())
	...
	_, _, err = keeper.Instantiate(ctx, codeID, walletA initMsgBz, nil, "some label", sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
	if err != nil {
		err = extractInnerError(t, err, nonce, false, false)
	}
...
func TestMultiSig(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, _, _, _, _ := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.NewCoins())

	initMsg := `{"nop":{}}`
	msg := types.ContractMsg{CodeHash: []byte(codeHash), Msg: []byte(initMsg)}
	...
	for i := 0; i < 5; i++ {
		for j := 0; j <= i; j++ {
			...
			contractAddressA, _, err := keeper.Instantiate(ctx, codeID, multisigAddr.address  initMsgBz, nil, label, sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
			if err != nil {
				err = extractInnerError(t, err, nonce, true)
			}
			require.NoError(t, err)
...
func TestMultiSigThreshold(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		for j := 0; j <= i; j++ {
			...
			// Reset wasm events
			ctx, keeper, codeID, _, _, _, _, _ = setupTest(t, "./testdata/test-contract/contract.wasm", sdk.NewCoins())
		}
	}
}
...
func TestMultiSigThresholdNotMet(t *testing.T) {
	...
	_, _, err = keeper.Instantiate(ctx, codeID, multisigAddr.address initMsgBz, nil, "demo contract 1", sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
	if err != nil {
		err = extractInnerError(t, err, nonce, false, false)
	}
...
func TestMultiSigExecute(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, _, _, walletB, privKeyB := setupTest(t, "./testdata/erc20.wasm", sdk.NewCoins())
	...
	_, err = keeper.Execute(ctx, contractAddress, multisigAccount.address, execMsgBz, funds, nil)
	if err != nil {
		err = extractInnerError(t, err, nonce, true)
	}
	require.NoError(t, err)
	...
	require.Equal(t,
		[]ContractEvent{
			{
				{Key: "contract_address", Value: contractAddress.Bytes()},
				{Key: "action", Value: []byte("transfer")},
				{Key: "sender", Value: multisigAccount.address.Bytes()},
				{Key: "recipient", Value: walletB.Bytes()},
			},
		},
		wasmEvents,
	)
...
func TestMultiSigInMultiSig(t *testing.T) {
	t.SkipNow() // skipping till multisig is fixed
	ctx, keeper, codeID, codeHash, _, privKeyA, _, privKeyB := setupTest(t, "./testdata/test-contract/contract.wasm", sdk.NewCoins())
	...
	sdkMsg := types.MsgInstantiateContract{
		Sender: multimultisigAccount.address.String(),
		// Admin:     nil,
		CodeID:     codeID,
		ContractId: "demo contract 1",
		Msg:        initMsgBz,
...
func TestInvalidKeyType(t *testing.T) {
	...
	_, _, err = keeper.Instantiate(ctx, codeID, edAddr, initMsgBz, nil, "demo contract 1", sdk.NewCoins(sdk.NewInt64Coin("denom", 0)), nil, 0, 0, time.Now())
	require.Contains(t, err.Error(), "failed to verify transaction signature")
}
...
func TestInvalidKeyTypeInMultisig(t *testing.T) {
	...
	sdkMsg := types.MsgInstantiateContract{
		Sender: multisigPubkey.address.String(),
		// Admin:     nil,
		CodeID:     codeID,
		ContractId: "demo contract 1",
		Msg:        initMsgBz,
...
func TestWrongFundsNoFunds(t *testing.T) {
	...
	_, _, err = keeper.Instantiate(ctx, codeID, walletA, initMsgBz, nil, "demo contract 1", sdk.NewCoins(sdk.NewInt64Coin("denom", 1000)), nil, 0, 0, time.Now())
	if err != nil {
		err = extractInnerError(t, err, nonce, false, false)
	}
	require.Error(t, err)
...
func TestWrongMessage(t *testing.T) {
	...
	notReallyTheMsg := types.ContractMsg{
		CodeHash: []byte(codeHash),
	...
	_, _, err = keeper.Instantiate(ctx, codeID, walletA, notTheRealMsgBz, nil, "demo contract 1", sdk.NewCoins(sdk.NewInt64Coin("denom", 1000)), nil, 0, 0, time.Now())
	if err != nil {
		err = extractInnerError(t, err, nonce, false, false)
	}
	require.Error(t, err)
...
func TestWrongContractAddress(t *testing.T) {
	t.SkipNow()
	ctx, keeper, codeID, codeHash, walletA, privKeyA, walletB, privKeyB := setupTest(t, "./testdata/erc20.wasm", sdk.NewCoins())
