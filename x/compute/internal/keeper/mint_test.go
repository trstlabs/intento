...
type MintExecMsgInflation struct {
	InflationRate MintMsg `json:"inflation_rate"`
}

type MintExecMsgBondedRatio struct {
	BondedRatio MintMsg `json:"bonded_ratio"`
}
...
func TestMintQuerier(t *testing.T) {
	...
	require.NoError(t, err)
	inter, _ := time.ParseDuration("100m")
	dur, _ := time.ParseDuration("1000m")
	govId, err := keeper.Create(ctx, creator, govCode, "", "", dur, inter, "test", "test")
	require.NoError(t, err)
	...
	govAddr, _, err := keeper.Instantiate(ctx, govId, creator, initBz, initBz, "gidi gov", nil, nil, dur, inter, start)
