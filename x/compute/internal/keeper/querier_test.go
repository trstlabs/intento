...
func TestQueryContractState(t *testing.T) {
	...
	specs := map[string]struct {
		...
		"query": {
			srcPath:     []string{QueryGetContractPrivateState, addr.String()},
		...
		/*
			"query raw key": {
				srcPath:          []string{QueryGetContractPrivateState, addr.String(), QueryMethodContractStateRaw},
		...
		*/
		/*"query smart": {
			srcPath:     []string{QueryGetContractPrivateState, addr.String(), QueryMethodContractStateSmart},
			srcReq:      abci.RequestQuery{Data: []byte(`{"verifier":{}}`)},
			expSmartRes: fmt.Sprintf(`{"verifier":"%s"}`, anyAddr.String()),
		},
		...
		*/
		"query with unknown address": {
			srcPath:     []string{QueryGetContractPrivateState, anyAddr.String()},
			expModelLen: 0,
...
func TestListContractByCodeOrdering(t *testing.T) {
	...
	codeID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "test", "test")
	require.NoError(t, err)
	...
	initMsgBz, _ = wasmCtx.Encrypt(msg.Serialize())
	...
	for i, contract := range contracts {
		require.Equal(t, fmt.Sprintf("contract %d", i), contract.ContractId)
		require.NotEmpty(t, contract.Address)
		// ensure these are not shown
		// assert.Nil(t, contract.Msg)
		assert.Nil(t, contract.Created)
	}
}

func TestQueryParams(t *testing.T) {
	ctx, keepers := CreateTestInput(t, false, SupportedFeatures, &MessageEncoders{}, nil)

	resp, err := NewGrpcQuerier(keepers.WasmKeeper).Params(sdk.WrapSDKContext(ctx), &types.QueryParamsRequest{})
	require.NoError(t, err)
