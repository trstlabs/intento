...
func handleMsgData(msgData *sdk.MsgData) (proto.Message, int, error) {
	...
	if !ok {
		return nil, -1, errorsmod.Wrapf(types.ErrAnyUnmarshal, "unknown message type") // unknown msg type
	}

	msgResponse := entry.NewResponse()
	if err := proto.Unmarshal(msgData.Data, msgResponse); err != nil {
		return nil, -1, errorsmod.Wrapf(types.ErrAnyUnmarshal, "cannot unmarshal response message: %s", err.Error())
	}
