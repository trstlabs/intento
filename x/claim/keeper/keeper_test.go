...
func (suite *KeeperTestSuite) SetupTest() {
	...
	suite.app.AppKeepers.AllocKeeper.SetParams(suite.ctx, alloctypes.DefaultParams())
	...
	suite.app.AppKeepers.ClaimKeeper.CreateModuleAccount(suite.ctx, sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(10000000)))
	startTime := time.Now()

	suite.msgSrvr = keeper.NewMsgServerImpl(*suite.app.AppKeepers.ClaimKeeper)
	suite.app.AppKeepers.ClaimKeeper.SetParams(suite.ctx, types.Params{
		//AirdropEnabled:     true,
...
func (s *KeeperTestSuite) TestModuleAccountCreated() {
	app, ctx := s.app, s.ctx
	moduleAddress := app.AppKeepers.AccountKeeper.GetModuleAddress(types.ModuleName)
	balance := app.AppKeepers.BankKeeper.GetBalance(ctx, moduleAddress, sdk.DefaultBondDenom)
...
func (s *KeeperTestSuite) TestClaimClaimable() {
	...
	s.app.AppKeepers.AccountKeeper.SetAccount(s.ctx, authtypes.NewBaseAccount(addr1, nil, 0, 0))
	s.app.AppKeepers.AccountKeeper.SetAccount(s.ctx, authtypes.NewBaseAccount(addr2, nil, 0, 0))

	s.app.AppKeepers.ClaimKeeper.SetParams(s.ctx, types.Params{
	...
	err := s.app.AppKeepers.ClaimKeeper.SetClaimRecords(s.ctx, claimRecords)
	...
	err = s.app.AppKeepers.ClaimKeeper.SetClaimRecord(s.ctx, record)
	s.Require().NoError(err)

	// addr1 has to delegate
	validator, err := stakingtypes.NewValidator(valAddr, pub1, stakingtypes.Description{})
	s.Require().NoError(err)
	validator = stakingkeeper.TestingUpdateValidator(*s.app.AppKeepers.StakingKeeper, s.ctx, validator, true)
	s.app.AppKeepers.StakingKeeper.AfterValidatorCreated(s.ctx, validator.GetOperator())

	validator, _ = validator.AddTokensFromDel(sdk.TokensFromConsensusPower(1, sdk.DefaultPowerReduction))

	_, err = s.app.AppKeepers.StakingKeeper.Delegate(s.ctx, addr1, record.InitialClaimableAmount.AmountOf(sdk.DefaultBondDenom).Quo(sdk.NewInt(20)), stakingtypes.Unbonded, validator, true)
	s.NoError(err)
	_, err = s.app.AppKeepers.StakingKeeper.Delegate(s.ctx, addr1, record.InitialClaimableAmount.AmountOf(sdk.DefaultBondDenom).Quo(sdk.NewInt(20)), stakingtypes.Unbonded, validator, true)
	s.NoError(err)
	...
	_, err = s.msgSrvr.ClaimClaimable(ctx, msgClaimClaimable)
	s.Require().NoError(err)
	claimedCoins := s.app.AppKeepers.BankKeeper.GetAllBalances(s.ctx, addr1)
	...
	s.app.AppKeepers.StakingKeeper.Delegate(s.ctx, addr1, sdk.NewInt(68), stakingtypes.Unbonded, validator, true)
	...
	delegationInfo := s.app.AppKeepers.StakingKeeper.GetAllDelegatorDelegations(s.ctx, addr1)
	totalDelegations := sdk.ZeroDec()
	for _, delegation := range delegationInfo {
		totalDelegations = totalDelegations.Add(delegation.Shares)
	}
	balanceCoins := s.app.AppKeepers.BankKeeper.GetAllBalances(s.ctx, addr1)
	...
	err = s.app.AppKeepers.ClaimKeeper.SetClaimRecord(s.ctx, record)
	s.Require().NoError(err)
	s.app.AppKeepers.StakingKeeper.Delegate(s.ctx, addr2, sdk.NewInt(200), stakingtypes.Unbonded, validator, true)
	s.app.AppKeepers.StakingKeeper.Delegate(s.ctx, addr2, sdk.NewInt(68), stakingtypes.Unbonded, validator, true)
	msgClaimClaimable = types.NewMsgClaimClaimable(addr2)
	_, err = s.msgSrvr.ClaimClaimable(ctx, msgClaimClaimable)
	s.Require().NoError(err)

	//get delegations and calculate min bonded ratio for claim
	delegationInfo = s.app.AppKeepers.StakingKeeper.GetAllDelegatorDelegations(s.ctx, addr2)
	totalDelegations = sdk.ZeroDec()
	for _, delegation := range delegationInfo {
		totalDelegations = totalDelegations.Add(delegation.Shares)
	}
	balanceCoins = s.app.AppKeepers.BankKeeper.GetAllBalances(s.ctx, addr2)
...
func (s *KeeperTestSuite) TestHookOfUnclaimableAccount() {
	...
	claim, err := s.app.AppKeepers.ClaimKeeper.GetClaimRecord(s.ctx, addr1)
	s.Contains(err.Error(), "address does not have claim record")
	s.Equal(types.ClaimRecord{}, claim)

	s.app.AppKeepers.ClaimKeeper.AfterDelegationModified(s.ctx, addr1, sdk.ValAddress(addr1))
...
func (s *KeeperTestSuite) TestHookBeforeAirdropStart() {
	...
	coins, err := s.app.AppKeepers.ClaimKeeper.GetTotalClaimableForAddr(s.ctx, addr1)
	s.NoError(err)
	// Now, it is before starting air drop, so this value should return the empty coins
	s.True(coins.Empty())

	coins, err = s.app.AppKeepers.ClaimKeeper.GetTotalClaimableAmountForAction(s.ctx, addr1, types.ActionDelegateStake)
	s.NoError(err)
	// Now, it is before starting air drop, so this value should return the empty coins
	s.True(coins.Empty())
...
func (s *KeeperTestSuite) TestAirdropDisabled() {
	...
	s.app.AppKeepers.ClaimKeeper.AfterGovernanceVoted(s.ctx, addr1)
	balances = s.app.AppKeepers.BankKeeper.GetAllBalances(s.ctx, addr1)
	...
	// add extra 2 hours
	s.app.AppKeepers.ClaimKeeper.AfterDelegationModified(s.ctx.WithBlockTime(airdropStartTime.Add(time.Hour*2)), addr1, sdk.ValAddress(addr1))
	balances = s.app.AppKeepers.BankKeeper.GetAllBalances(s.ctx, addr1)
	// Now, it is the time for air drop, so claim module should send the balances to the user after delegate.
	s.Equal(claimRecords[0].InitialClaimableAmount.AmountOf(sdk.DefaultBondDenom).Quo(sdk.NewInt(int64(len(types.Action_value)))).Quo(sdk.NewInt(5)), balances.AmountOf(sdk.DefaultBondDenom))
}
...
func (s *KeeperTestSuite) TestDuplicatedActionNotWithdrawRepeatedly() {
	...
	coins1, err := s.app.AppKeepers.ClaimKeeper.GetTotalClaimableForAddr(s.ctx, addr1)
	s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestNotRunningGenesisBlock() {
	...
	s.app.AppKeepers.ClaimKeeper.CreateModuleAccount(s.ctx, sdk.NewCoin(sdk.DefaultBondDenom, sdk.NewInt(10000000)))
	// set airdrop enabled but with date in the future
	s.app.AppKeepers.ClaimKeeper.SetParams(s.ctx, types.Params{
		//AirdropEnabled:     true,
		AirdropStartTime:       time.Now().Add(time.Hour * -1),
...
func (s *KeeperTestSuite) TestEndAirdrop() {
	...
	err = s.app.AppKeepers.ClaimKeeper.EndAirdrop(s.ctx)
	s.Require().NoError(err)

	moduleAccAddr := s.app.AppKeepers.AccountKeeper.GetModuleAddress(types.ModuleName)
	coins := s.app.AppKeepers.BankKeeper.GetBalance(s.ctx, moduleAccAddr, sdk.DefaultBondDenom)
