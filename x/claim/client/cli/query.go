...
Example:
$ %s query claim claimable-for-action stars1ey69r37gfxvxg62sh4r0ktpuc46pzjrmkkj479 ActionItemCreated
`,
				version.AppName,
			),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !ok {
				return fmt.Errorf("invalid Action type: %s.  Valid actions are %s, %s, %s, %s", args[1],
					types.ActionComputeExecute, types.ActionComputeInstantiate, types.ActionGovernanceVote /*types.ActionItemBought, */, types.ActionDelegateStake)
			}
...
Example:
$ %s query claim total-claimable stars1ey69r37gfxvxg62sh4r0ktpuc46pzjrmkkj479
`,
				version.AppName,
			),
		),
