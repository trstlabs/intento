...
func getSubmitAutoTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg, err := types.NewMsgSubmitAutoTx(clientCtx.GetFromAddress().String(), txMsg, viper.GetString(FlagConnectionID), viper.GetString(flagInterval), viper.GetString(flagDuration), viper.GetUint64(flagStartAt))
			if err != nil {
				return err
			}
