...
func (k Keeper) VerifyKeyProof(ctx sdk.Context, msg *types.MsgSubmitQueryResponse, query types.Query) error {
	...
	if len(msg.Result) != 0 {
		...
		k.Logger(ctx).Info(LogICQCallbackWithHostChain(query.ChainId, query.CallbackId, "Inclusion proof validated - QueryId %s", query.Id))
		...
		k.Logger(ctx).Info(LogICQCallbackWithHostChain(query.ChainId, query.CallbackId, "Non-inclusion proof validated - QueryId %s", query.Id))
	}
...
func (k Keeper) HandleQueryTimeout(ctx sdk.Context, msg *types.MsgSubmitQueryResponse, query types.Query) error {
	...
	case types.TimeoutPolicy_REJECT_QUERY_RESPONSE:
		k.Logger(ctx).Info(LogICQCallbackWithHostChain(query.ChainId, query.CallbackId, "Rejecting query"))
		return nil

	case types.TimeoutPolicy_RETRY_QUERY_REQUEST:
		k.Logger(ctx).Info(LogICQCallbackWithHostChain(query.ChainId, query.CallbackId, "Retrying query..."))
		return k.RetryICQRequest(ctx, query)

	case types.TimeoutPolicy_EXECUTE_QUERY_CALLBACK:
		k.Logger(ctx).Info(LogICQCallbackWithHostChain(query.ChainId, query.CallbackId, "Executing callback..."))
