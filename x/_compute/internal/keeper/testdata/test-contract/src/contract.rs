...
pub fn instantiate(
    ...
    match msg {
        ...
        InstantiateMsg::AddPlaintextAttributes {} => Ok(Response::new()
            .add_plaintext_log("attr1", "🦄")
            .add_plaintext_log("attr2", "🌈")),
        ...
        InstantiateMsg::AddMixedAttributesAndEvents {} => Ok(Response::new()
            ...
            .add_plaintext_log("attr3", "🐙")
            .add_plaintext_log("attr4", "🦄")),
        ...
        InstantiateMsg::CallbackContractError {
            ...
            .add_log("init with a callback with contract error", "🤷‍♀️")),
        ...
        InstantiateMsg::CallbackToInit { code_id, code_hash } => Ok(Response::new()
            .add_message(CosmosMsg::Wasm(WasmMsg::Instantiate {
                ...
                code_hash,
                funds: vec![],
                contract_id: String::from("fi"),
            }))
            .add_log("instantiating a new contract from init!", "🐙")),
        ...
        InstantiateMsg::CallToQuery {
            ...
            let answer: u32 = deps
                .querier
                .query(&QueryRequest::Wasm(WasmQuery::Private {
                    contract_addr: addr,
                    code_hash,
...
pub fn execute(deps: DepsMut, env: Env, info: MessageInfo, msg: ExecuteMsg) -> StdResult<Response> {
    match msg {
        ...
        ExecuteMsg::InitV10 {
            ...
            resp.messages.push(SubMsg {
                ...
                msg: CosmosMsg::Wasm(WasmMsg::InstantiateAuto {
                    ...
                    msg: Binary::from(msg.as_bytes().to_vec()),
                    funds: vec![],
                    contract_id: "new2231231".to_string(),
                    code_id,
                }),
        ...
        ExecuteMsg::QueryV10WithError { address, code_hash } => {
            deps.querier.query(&QueryRequest::Wasm(WasmQuery::Private {
                code_hash,
                contract_addr: address,
        ...
        ExecuteMsg::LogMsgSender {} => {
            Ok(Response::new().add_log("msg.sender", info.sender.as_str()))
        }
        ...
        ExecuteMsg::Secp256k1VerifyFromCrate {
            ...
            for _ in 0..iterations {
                ...
                res = match secp256k1_verifier.verify(
                    ...
                    Ok(()) => Ok(Response::new().add_log("result", "true")),
                    Err(_err) => Ok(Response::new().add_log("result", "false")),
                };
            }
            ...
            for _ in 0..iterations {
                res = match deps.api.secp256k1_recover_pubkey(
                    ...
                    Ok(result) => Ok(Response::new()
                        .add_log("result", format!("{}", Binary(result).to_base64()))),
                    Err(err) => Err(StdError::generic_err(format!("{:?}", err))),
                };
            }
...
pub fn send_multiple_sub_messages(env: Env, _deps: DepsMut) -> StdResult<Response> {
    ...
    resp.messages.push(SubMsg {
        ...
        msg: CosmosMsg::Wasm(WasmMsg::InstantiateAuto {
            ...
            funds: vec![],
            contract_id: "new202213".to_string(),
            code_id: 1,
        }),
        gas_limit: Some(10000000_u64),
        reply_on: ReplyOn::Always,
...
pub fn init_new_contract_with_error(env: Env, _deps: DepsMut) -> StdResult<Response> {
    ...
    resp.messages.push(SubMsg {
        ...
        msg: CosmosMsg::Wasm(WasmMsg::Instantiate {
            ...
            funds: vec![],
            contract_id: "new2022133".to_string(),
            code_id: 1,
        }),
        gas_limit: Some(10000000_u64),
        reply_on: ReplyOn::Always,
...
pub fn reply(deps: DepsMut, env: Env, reply: Reply) -> StdResult<Response> {
    match (reply.id, reply.result) {
        ...
        (2300, SubMsgResult::Ok(_)) => Ok(Response::new()
            .add_plaintext_log("attr5", "🤯")
            .add_plaintext_log("attr6", "🦄")),
...
fn send_external_query_panic(
    ...
    let err = deps
        .querier
        .query::<u8>(&QueryRequest::Wasm(WasmQuery::Private {
            contract_addr,
...
fn send_external_query_stderror(
    ...
    let answer = deps
        .querier
        .query::<Binary>(&QueryRequest::Wasm(WasmQuery::Private {
            contract_addr,
            msg: Binary::from(
...
fn send_external_query_bad_abi_receiver(
    ...
    let answer = deps
        .querier
        .query::<String>(&QueryRequest::Wasm(WasmQuery::Private {
            contract_addr,
    ...
    match answer {
        Ok(wtf) => Ok(Response::new().add_log("wtf", wtf)),
        Err(e) => Err(e),
    }
}
...
pub fn exec_callback_to_init(
    ...
    Response::new()
        .add_message(CosmosMsg::Wasm(WasmMsg::Instantiate {
            ...
            code_hash,
            funds: vec![],
            contract_id: String::from("hi"),
        }))
        .add_log("instantiating a new contract", "🪂")
}
...
fn exec_with_callback_contract_error(contract_addr: String, code_hash: String) -> Response {
    Response::new()
        ...
        .add_log("exec with a callback with contract error", "🤷‍♂️")
}
