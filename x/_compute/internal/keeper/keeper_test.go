...
func TestCreate(t *testing.T) {
	...
	contractID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
...
func TestCreateDuplicate(t *testing.T) {
	...
	// create second copy
	duplicateID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
...
func TestInstantiate(t *testing.T) {
	...
	contractID, err := keeper.Create(ctx, creator, wasmCode, "https://github.com/trstlabs/trst/blob/master/cosmwasm/contracts/hackatom/src/contract.rs", "", 0, 0, "title", "descr")
	require.NoError(t, err)
...
func TestInstantiateWithDeposit(t *testing.T) {
	...
	for msg, spec := range specs {
		t.Run(msg, func(t *testing.T) {
			ctx, keeper, codeID, _, _, _, _, _ := setupTest(t, "./testdata/hackatom.wasm", sdk.NewCoins())
			...
			initMsgBz, err := json.Marshal(Msg{Verifier: fred, Beneficiary: bob})
			require.NoError(t, err)
			...
			// when
			_, _, addr, _, err := initHelperImpl(t, keeper, ctx, codeID, bob, bobPriv, string(initMsgBz), false, defaultGasForTests, wasmCalls, sdk.NewCoins(sdk.NewInt64Coin("denom", int64(deposit))))
			// then
			if spec.expError {
				require.Error(t, err)
				return
	...
	func TestInstantiateWithPermissions(t *testing.T) {
		...
		for msg, spec := range specs {
			t.Run(msg, func(t *testing.T) {
				...
				assert.True(t, spec.expError.Is(err), "got %+v\n", err)
			})
		}
	}
*/
...
func TestExecute(t *testing.T) {
	...
	require.Equal(t, "trust18vd8fpwxzck93qlwghaj6arh4p7c5n894lxvdh", addr.String())
	...
	_, _, _, _, _, trialExecErr := execHelper(t, keeper, trialCtx, addr, creator, creatorPrivKey, `{"release":{}}`, true, defaultGasForTests, 0)
...
func TestExecuteWithDeposit(t *testing.T) {
	...
	for msg, spec := range specs {
		t.Run(msg, func(t *testing.T) {
			...
			initMsgBz, err := json.Marshal(Msg{Verifier: bob, Beneficiary: fred})
			require.NoError(t, err)

			_, _, contractAddr, _, err := initHelperImpl(t, keeper, ctx, codeID, bob, bobPriv, string(initMsgBz), true, defaultGasForTests, -1, sdk.NewCoins())
			require.Empty(t, err)
			...
			// when
			_, _, _, _, _, err = execHelperImpl(t, keeper, ctx, contractAddr, bob, bobPriv, `{"release":{}}`, false, defaultGasForTests, deposit, wasmCalls)
...
func TestExecuteWithNonExistingAddress(t *testing.T) {
	...
	executeMsg := types.MsgExecuteContract{
		Sender:   creator.String(),
		Contract: nonExistingAddress.String(),
		Msg:      msgBz,
		Funds:    nil,
	}
...
func TestExecuteWithPanic(t *testing.T) {
	...
	_, _, addr, _, err := initHelper(t, keeper, ctx, contractID, creator, creatorPrivKey, string(initMsgBz), false, defaultGasForTests)
...
func TestExecuteWithStorageLoop(t *testing.T) {
	...
	initMsgBz, _ := json.Marshal(initMsg)
...
func TestMigrate(t *testing.T) {
	...
	originalCodeID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
	newCodeID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
	...
	for msg, spec := range specs {
		t.Run(msg, func(t *testing.T) {
			...
			require.True(t, spec.expErr.Is(err), "expected %v but got %+v\n", spec.expErr, err)
			if spec.expErr != nil {
				return
			}
...
func TestMigrateWithDispatchedMessage(t *testing.T) {
	...
	originalContractID, err := keeper.Create(ctx, creator, wasmCode, "", "", 0, 0, "title", "descr")
	require.NoError(t, err)
