...
func GenesisInstantiateContractCmd(defaultNodeHome string, genesisMutator GenesisMutator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "instantiate-contract [code_id_int64] [json_encoded_init_args] --contractId [text] --run-as [address] --amount [coins,optional]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			clientCtx.FromAddress = senderAddr
			msg, err := parseInstantiateArgs(args, clientCtx, cmd.Flags())
			if err != nil {
				return err
			}
			if err = msg.ValidateBasic(); err != nil {
...
func GenesisExecuteContractCmd(defaultNodeHome string, genesisMutator GenesisMutator) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg, err := parseExecuteArgs(cmd, address, execMsg, amountStr, false, "", "", clientCtx)
			if err != nil {
				return err
			}
			if err = msg.ValidateBasic(); err != nil {
...
func getAllContracts(state *types.GenesisState) []contractMeta {
	...
	for i, c := range state.Contracts {
		all[i] = contractMeta{
			ContractAddress: c.ContractAddress.String(),
			Info:            c.ContractInfo,
		}
	}
	...
	for _, m := range state.GenMsgs {
		if msg := m.GetInstantiateContract(); msg != nil {
			...
			all = append(all, contractMeta{
				ContractAddress: buildContractAddress(msg.CodeID, seq).String(),
...
func hasContract(state *types.GenesisState, contractAddr string) bool {
	for _, c := range state.Contracts {
		if c.ContractAddress.String() == contractAddr {
			return true
		}
	}
	seq := contractSeqValue(state)
	for _, m := range state.GenMsgs {
		if msg := m.GetInstantiateContract(); msg != nil {
			if buildContractAddress(msg.CodeID, seq).String() == contractAddr {
				return true
			}
			seq++
		}
...
// buildContractAddress builds an sdk account address for a contract given a codeID and instanceID
