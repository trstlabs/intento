...
build_cli:
	go build -o trstd -mod=readonly -tags "$(GO_TAGS) trstd" -ldflags '$(LD_FLAGS)' ./cmd/trstd 

xgo_build_trstd: go.sum
	@echo "--> WARNING! This builds from origin/$(CURRENT_BRANCH)!"
	xgo --image techknowlogick/xgo:go-1.15.15 --targets $(XGO_TARGET) -tags="$(GO_TAGS) trstd" -ldflags '$(LD_FLAGS)' --branch "$(CURRENT_BRANCH)" github.com/trstlabs/trst/cmd/trstd 
...
build_windows_cli:
	$(MAKE) xgo_build_trstd XGO_TARGET=windows/amd64
	mv trstd -windows-* trstd-windows-amd64.exe

build_macos_cli:
	$(MAKE) xgo_build_trstd XGO_TARGET=darwin/amd64
	mv trstd -darwin-* trstd-macos-amd64

build_linux_cli:
	$(MAKE) xgo_build_trstd XGO_TARGET=linux/amd64
	mv trstd -linux-amd64 trstd-linux-amd64

build_linux_arm64_cli:
	$(MAKE) xgo_build_trstd XGO_TARGET=linux/arm64
	mv trstd -linux-arm64 trstd-linux-arm64
...
build-dev-image:
	...
	docker build --build-arg SGX_MODE=SW --build-arg TRUST_NODE_TYPE=BOOTSTRAP --build-arg CHAIN_ID=trstd ev-1 -f deployment/dockerfiles/release.Dockerfile -t build-release .
	docker build --build-arg SGX_MODE=SW --build-arg TRUST_NODE_TYPE=BOOTSTRAP --build-arg CHAIN_ID=trstd ev-1 -f deployment/dockerfiles/dev-image.Dockerfile -t trstlabs/trst-sw-dev:${DOCKER_TAG} .
...
build-custom-dev-image:
	...
	docker build --build-arg SGX_MODE=SW --build-arg TRUST_NODE_TYPE=BOOTSTRAP -f deployment/dockerfiles/custom-node.Dockerfile -t trstlabs/trst-sw-dev-custom-bootstrap:${DOCKER_TAG} .
	docker build --build-arg SGX_MODE=SW --build-arg TRUST_NODE_TYPE=NODE -f deployment/dockerfiles/custom-node.Dockerfile -t trstlabs/trst-sw-dev-custom-node:${DOCKER_TAG} .
...
build-testnet: docker_base
	...
	docker build --build-arg BUILD_VERSION=${VERSION} --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=BOOTSTRAP -f deployment/dockerfiles/release.Dockerfile -t trstlabs/trst-bootstrap:v$(VERSION)-testnet .
	docker build --build-arg BUILD_VERSION=${VERSION} --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=NODE -f deployment/dockerfiles/release.Dockerfile -t trstlabs/trst-node:v$(VERSION)-testnet .
...
build-mainnet:
	...
	docker build --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=BOOTSTRAP -f deployment/dockerfiles/release.Dockerfile -t trstlabs/trst-bootstrap:v$(VERSION)-mainnet .
	docker build --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=NODE -f deployment/dockerfiles/release.Dockerfile -t trstlabs/trst-node:v$(VERSION)-mainnet .
	docker build --build-arg BUILD_VERSION=${VERSION} --build-arg SGX_MODE=HW -f deployment/dockerfiles/build-deb.Dockerfile -t deb_build .
	docker run -e VERSION=${VERSION} -v $(CUR_DIR)/build:/build deb_build
...
docker_bootstrap: docker_base
	docker build --build-arg SGX_MODE=${SGX_MODE} --build-arg TRUST_NODE_TYPE=BOOTSTRAP -f deployment/dockerfiles/local-node.Dockerfile -t trstlabs/trst-bootstrap-${ext}:${DOCKER_TAG} .

docker_node: docker_base
	docker build --build-arg SGX_MODE=${SGX_MODE} --build-arg TRUST_NODE_TYPE=NODE -f deployment/dockerfiles/local-node.Dockerfile -t trstlabs/trst-node-${ext}:${DOCKER_TAG} .

docker_local_azure_hw: docker_base
	docker build --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=NODE -f deployment/dockerfiles/local-node.Dockerfile -t ci-enigma-sgx-node .
	docker build --build-arg SGX_MODE=HW --build-arg TRUST_NODE_TYPE=BOOTSTRAP -f deployment/dockerfiles/local-node.Dockerfile -t ci-enigma-sgx-bootstrap .
...
go-tests: build-test-contract
	...
	GOMAXPROCS=8 SGX_MODE=SW go test -failfast -timeout 1200s -v ./x/compute/internal/... $(GO_TEST_ARGS)
...
secret-contract-optimizer:
	docker build -f deployment/dockerfiles/secret-contract-optimizer.Dockerfile -t trstlabs/secret-contract-optimizer:${TAG} .
	docker tag trstlabs/secret-contract-optimizer:${TAG} trstlabs/secret-contract-optimizer:latest
