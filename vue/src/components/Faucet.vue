  ...
  methods: {
    ...
    onCaptchaVerified: async function (recaptchaToken) {
      let accountQuery = await axios.get(
        "https://node.trustpriceprotocol.com/auth/accounts/" + this.address
      );
      ...
      if (!accountQuery.data.result.value.address) {
        ...
        alert(
          "Account seems to already exist on the TPP blockchain, try signing in, or create a new account"
        );
      }
    },
