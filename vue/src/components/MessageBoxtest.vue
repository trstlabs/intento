...
<form @submit.prevent="createRoom" v-if="addNewRoom">
	...
	<chat-window
			...
			:styles="styles"
			:current-user-id="this.$store.state.account.address"
			:room-id="roomId"
			:rooms="loadedRooms"
  ...
  MESSAGE_ID_1: {
    ...
    async addData() {
		...
		async fetchMoreRooms() {
			...
			let query = roomsRef
				.where('users', 'array-contains', this.$store.state.account.address)
			...
			Object.keys(roomList).forEach(key => {
				...
				const roomContacts = room.users.filter(
					user => user._id !== this.$store.state.account.address
				)
    ...
    src="img/brand/icon.png"
		...
		formatLastMessage(message) {
			...
			return {
				...
				...{
					...
					distributed: true,
					seen: message.sender_id === this.$store.state.account.address ? message.seen : null,
					new:
						message.sender_id !== this.$store.state.account.address &&
						(!message.seen || !message.seen[this.$store.state.account.address])
				}
			}
		},
		...
		markMessagesSeen(room, message) {
			if (
				message.data().sender_id !== this.$store.state.account.address &&
				(!message.data().seen || !message.data().seen[this.$store.state.account.address])
			) {
				messagesRef(room.roomId)
					.doc(message.id)
					.update({
						[`seen.${this.$store.state.account.address}`]: new Date()
					})
			}
		},
		...
		async sendMessage({ content, roomId, file, replyMessage }) {
			const message = {
				sender_id: this.$store.state.account.address,
				content,
				timestamp: new Date()
			}
		...
		async openUserTag({ user }) {
			...
			this.rooms.forEach(room => {
				if (room.users.length === 2) {
					...
					if (
						(userId1 === user._id || userId1 === this.$store.state.account.address) &&
						(userId2 === user._id || userId2 === this.$store.state.account.address)
					) {
						roomId = room.roomId
					}
				}
			...
			const query1 = await roomsRef
				.where('users', '==', [this.$store.state.account.address, user._id])
				.get()
			...
			let query2 = await roomsRef
				.where('users', '==', [user._id, this.$store.state.account.address])
				.get()
			...
			const room = await roomsRef.add({
				users: [user._id, this.$store.state.account.address],
				lastUpdated: new Date()
			})
		...
		async deleteMessage({ message, roomId }) {
			...
			if (file) {
				const deleteFileRef = filesRef
					.child(this.$store.state.account.address)
					.child(message._id)
		...
		async sendMessageReaction({ reaction, remove, messageId, roomId }) {
			const dbAction = remove
				? firebase.firestore.FieldValue.arrayRemove(this.$store.state.account.address)
				: firebase.firestore.FieldValue.arrayUnion(this.$store.state.account.address)
      ...
      console.log(firebase.database())
			const userStatusRef = firebase.database().ref('/status/' + this.$store.state.account.address)
