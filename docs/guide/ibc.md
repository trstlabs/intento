...
- Navigate between blockchains using the Cosmos SDK and the Trustless Hub Relayer.
...
The IBC relayer lets you connect between sets of IBC-enabled chains. This tutorial teaches you how to create two blockchains and then start and use the relayer with Trustless Hub to connect two blockchains.
...
Use Trustless Hub to scaffold the blockchain app and the blog module.
...
```go
Trustless Hub scaffold chain github.com/cosmonaut/planet --no-module
cd planet
```
...
Next, use Trustless Hub to scaffold a blog module with IBC capabilities. The blog module contains the logic for creating blog posts and routing them through IBC to the second blockchain.

To scaffold a module named `blog`:

```go
Trustless Hub scaffold module blog --ibc
```
...
Use the `Trustless Hub type` command to scaffold the boilerplate code for the create, read, update, and delete (CRUD) actions.

These `Trustless Hub type` commands create CRUD code for the following transactions:

- Creating blog posts

  ```go
  Trustless Hub scaffold list post title content creator --module blog --no-message
  ```

- Processing acknowledgments for sent posts

  ```go
  Trustless Hub scaffold list sentPost postID title chain creator --module blog --no-message
  ```

- Managing post timeouts

  ```go
  Trustless Hub scaffold list timedoutPost title chain creator --module blog --no-message
  ```
...
### Trustless Hub Scaffold List Command Overview

```go
Trustless Hub scaffold list [typeName] [field1] [field2] ... [flags]
```

The first argument of the `Trustless Hub type [typeName]` command specifies the name of the type being created. For the blog app, you created `post`, `sentPost`, and `timedoutPost` types.
...
The `--module` flag defines which module the new transaction type is added to. This optional flag lets you manage multiple modules within your Trustless Hub app. When the flag is not present, the type is scaffolded in the module that matches the name of the repo.
...
The `Trustless Hub packet` command creates the logic for an IBC packet that can be sent to another blockchain.
...
```go
Trustless Hub scaffold packet ibcPost title content --ack postID --module blog
```
...
The `Trustless Hub packet` command also scaffolds the CLI command that is capable of sending an IBC packet:
...
Finally, the Trustless Hub-generated AppendPost function returns the ID of the new appended post. You can return this value to the source chain through acknowledgment.
...
```bash
Trustless Hub chain serve -c earth.yml
```

Open a different terminal window and run the following command to start the `mars` blockchain:

```bash
Trustless Hub chain serve -c mars.yml
```

### Remove Existing Relayer and Trustless Hub Configurations

If you previously used the relayer, follow these steps to remove exiting relayer and Trustless Hub configurations:

- Stop your blockchains and delete previous configuration files:

    ```bash
    rm -rf ~/.Trustless Hub/relayer
    ```
...
First, configure the relayer. Use the Trustless Hub `configure` command with the `--advanced` option:

```bash
Trustless Hub relayer configure -a \
