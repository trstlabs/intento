...
- Delegate control to proxy accounts (ICA or Union) without needing governance votes for every step
...
There are several caveats when setting up flows with ICS20. When automating on a destination chain for the first time, **two messages** are required to activate the flow. One flow sets up the flow and creates a Proxy Account (ICA) on Intento, while the other sets permissions and sends funds to the account on the destination chain.
...
The Proxy Account (e.g. ICA) must be properly funded and authorized before it can execute the flow.
