...
- IBC-Native: Set coordinated actions across blockchains using Interchain Accounts, or a Non-custiodal Hosted Account that abstracts away funds

- Submit actions from anywhere by sending tokens to Intento with an Action in the memo field.
    
- Modular and Secure: Built using the Cosmos SDK, secured by Celestia
    
- Privileged Execution: Intento can execute actions on behalf of your account by granting permissions to a non-custidial execution address
