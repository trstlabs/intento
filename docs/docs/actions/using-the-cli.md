...
`intentod tx autoibctx submit-auto-tx` takes one argument, a JSON-encoded Cosmos message. `@type` can be retrieved from the destination chain's `/proto` directory.
...
| `label`                        | Custom label for the AutoTx, such as transaction type or operation name. Optional.        | `AutoTransfer`    |
| `duration`                     | Duration for which the AutoTx remains active. Optional.                                   | `48h`             |
| `interval`                     | Custom interval between AutoTx executions. Optional.                                      | `2h`              |
| `start-at`                     | Custom start time for the AutoTx in UNIX time format. Optional.                           | `1625097600`      |
| `fee-funds`                    | Coins sent to limit the fees incurred during transaction execution. Optional.             | `100atom`         |
| `end-at`                       | Custom end time for the AutoTx in UNIX time format. Optional.                             | `1625184000`      |
| `updating-disabled`            | Disables future updates to the AutoTx configuration.                                      | `false`           |
...
| `stop-on-success`              | Stops execution of the AutoTx after a successful transaction.                             | `true`            |
| `stop-on-failure`              | Stops execution of the AutoTx after a failed transaction.                                 | `true`            |
...
| `reregister_ica_after_timeout` | Allows AutoTx to continue execution after an IBC channel timeout. Recommended.            | `true`            |
