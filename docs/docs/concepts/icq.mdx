...
1. **Supported Types:** You can use one of the [supported types](../reference/intent-engine/supported_types.md), which are predefined and commonly used data types in most ICQ compatible chains.
...
### **2. Trend Detection (Acceleration)**

**Why**: Identify when momentum is building up (acceleration). Only act when the rate of price change is increasing (Δ₂ > Δ₁).
**How**: Use `operand: "delta"` with `difference_mode: true`.
...
"comparisons": [
  {
    ...
    "operand": "delta", // Compares current delta vs previous delta
    "difference_mode": true,
    "icq_config": {
      "connection_id": "connection-0",
      "chain_id": "osmosis-1",
...
**How**: Spot smaller than TWAP → asset is cheap compared to average.
...
* Use **`operand="delta"`** to detect acceleration (compare current rate vs previous rate).
