...
use serde_json::json;
use enclave_cosmwasm_types::ibc::{IbcReceiveResponse,IbcChannelOpenResponse,IbcPacketReceiveMsg,Ibc3ChannelOpenResponse};
use sha2::Digest;
...
enum WasmOutput {
    ...
    OkIBCOpenChannel {
        ok: Option<IbcChannelOpenResponse>,
    },
}
...
pub fn encrypt_output(
    ...
    match &mut output {
        ...
        WasmOutput::OkIBCOpenChannel { ok } =>  { ok = match ok {
            Some(o) => { o = match o {
                Some(o) => &mut  Some(o.version),
                None =>&mut  Some("".to_string()),
            }}
            None =>&mut  Some("".to_string()),
        }
        /*ok=  &mut Some( Some(Ibc3ChannelOpenResponse {
                ok: match ok {
                    Some(o) =>  Some(o.unwrap().version),
                    None => Some("".to_string()),
                },
            }))*/
        }
