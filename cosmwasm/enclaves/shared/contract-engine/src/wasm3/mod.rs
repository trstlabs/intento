...
#[cfg(not(feature = "query-only"))]
use crate::gas::WRITE_BASE_GAS;

use crate::gas::{WasmCosts, READ_BASE_GAS};
...
impl Engine {
    ...
    pub fn new(
        ...
        let wrapped_code = create_module_instance(contract_code, &gas_costs, operation)?;
    ...
    pub fn init(&mut self, env: &FullEnv, msg: Vec<u8>) -> Result<Vec<u8>, EnclaveError> {
        self.with_instance(|instance, context| {
            ...
            let result = init.call_with_context(context, args);
    ...
    pub fn query(&mut self, env: &FullEnv, msg: Vec<u8>) -> Result<Vec<u8>, EnclaveError> {
        self.with_instance(|instance, context| {
            let msg_ptr = write_to_memory(instance, &msg)?;
            ...
            let result = query.call_with_context(context, args);
